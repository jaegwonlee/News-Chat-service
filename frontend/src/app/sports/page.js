...
export default function SportsPage() {
  ...
  // const { user, loading, logout } = useAuth(); // Header가 AuthContext를 사용하므로 여기서 직접 필요하지 않습니다.

  useEffect(() => {
    const fetchArticles = async () => {
      try {
        setIsLoading(true);
        const res = await fetch("http://localhost:8000/api/articles/category/스포츠");
  ...
  return (
    <div className="grid-container">
      {/* 2. 기존의 긴 <header>...</header> 부분을 아래 한 줄로 교체합니다. */}
      <Header />
